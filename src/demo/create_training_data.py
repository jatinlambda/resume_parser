...
from src.parser_classes.dl_based_parser import line_types, line_labels
...
def command_line_annotate(training_data_dir_path, index, file_path, file_content):
    try:
        ...
        with open(os.path.join(training_data_dir_path, file_path + '.txt')) as f:
            pass
    except IOError:
        with open(os.path.join(training_data_dir_path, file_path + '.txt'), 'wt', encoding='utf8') as f:
...
def main():
    ...
    training_data_dir_path = current_dir + '/../../dataset/training_data'
    data_dir_path = current_dir + '/../../dataset'
    ...
    collected = read_pdf_and_docx(data_dir_path, command_logging=True, callback=lambda index, file_path, file_content: {
        command_line_annotate(training_data_dir_path, index, file_path, file_content)
    })
